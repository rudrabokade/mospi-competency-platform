...
class User(Base):
    ...
    qualifications: Mapped[list[str] | None] = mapped_column(JSON, default=list)
...
class Course(Base):
    ...
    description: Mapped[str | None] = mapped_column(Text)
    skill_tags: Mapped[list[str] | None] = mapped_column(JSON, default=list)
...
class Quiz(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    material_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("materials.id"), nullable=False)
    title: Mapped[str] = mapped_column(String(500), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
