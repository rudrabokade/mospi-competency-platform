...
async def lifespan(app: FastAPI):
    ...
    async with engine.begin() as conn:
        # Enable vector extension only if available in PostgreSQL
        vector_res = await conn.execute(__import__("sqlalchemy").text("SELECT 1 FROM pg_available_extensions WHERE name = 'vector'"))
        if vector_res.scalar():
            await conn.execute(__import__("sqlalchemy").text("CREATE EXTENSION IF NOT EXISTS vector"))
            logger.info("pgvector extension enabled.")
        else:
            logger.info("pgvector extension not installed in system PostgreSQL; using scikit-learn embeddings.")
